...
/* PLAYER 테이블에서 NICKNAME이 NULL 인 선수 검색 */
SELECT * FROM PLAYER p
WHERE NICKNAME IS NULL;

SELECT PLAYER_NAME, NVL(NICKNAME, '없음') FROM PLAYER p
WHERE NICKNAME IS NULL;

/* PLAYER 테이블에서 POSITION이 NULL 인 선수 검색 */
SELECT * FROM PLAYER p
WHERE "POSITION" IS NULL;

/* PLAYER 테이블에서 POSITION이 NULL 인 선수를 '미정'으로 변경 후 검색 */
SELECT PLAYER_NAME, NVL(NICKNAME, '미정') FROM PLAYER p
WHERE "POSITION" IS NULL;

/* PLAYER 테이블에서 NATION이 등록되어 있으면 '등록', 아니면 '미등록'으로 검색 */
SELECT PLAYER_NAME, NVL2(NATION, '등록', '미등록') AS NATION FROM PLAYER p;

/* ===================================================================== */
/* AS(ALIAS) : 별칭 */
-- SELECT와 FROM 절에서 사용가능
-- SELECT절에서는 컬럼명 뒤에 띄어쓰고 작성하거나, AS뒤에 작성한다.
...
/* 위 결과에서 NICKNAME이 NULL 일 경우 "없음" 으로 변경 */
...
SELECT PLAYER_NAME || '의 별명은 ' || NVL(NICKNAME, '없음') || ' 이다' AS 자기소개 FROM PLAYER;
...
SELECT PLAYER_NAME || '의 포지션은 ' || NVL("POSITION", '미정') || '입니다' AS 자기소개 FROM PLAYER;
...
SELECT TEAM_NAME "팀 이름" FROM TEAM
WHERE TEAM_NAME LIKE '%천마';

/*PLAYER 테이블에서 김씨 찾기*/
SELECT PLAYER_NAME "선수 이름" FROM PLAYER
WHERE PLAYER_NAME LIKE '김%' AND NATION IS NULL;

/*PLAYER 테이블에서 김씨 두 자 찾기*/
SELECT PLAYER_NAME "선수 이름" FROM PLAYER
WHERE PLAYER_NAME LIKE '김_' AND NATION IS NULL;

/*PLAYER 테이블에서 김씨와 이씨 찾기*/
SELECT PLAYER_NAME "선수 이름" FROM PLAYER
WHERE (PLAYER_NAME LIKE '김%' OR PLAYER_NAME LIKE '이%') AND NATION IS NULL;

/*PLAYER 테이블에서 이씨가 아닌 사람 찾기(NOT)*/
SELECT PLAYER_NAME "선수 이름" FROM PLAYER
WHERE PLAYER_NAME NOT LIKE '이%' AND NATION IS NULL;
...
/* 키 순 정렬 */
SELECT PLAYER_NAME, HEIGHT, WEIGHT FROM PLAYER p
WHERE HEIGHT IS NOT NULL 
ORDER BY HEIGHT DESC, WEIGHT DESC;


/*
 * GROUP BY : ~별 (예 : 포지션 별 평균 키) --▶ 묶기
 * 
...
SELECT "POSITION", COUNT("POSITION") AS "POSITION COUNT" FROM PLAYER
WHERE "POSITION" IS NOT NULL 
GROUP BY "POSITION";

/* PLAYER 테이블에서 몸무게가 80이상인 선수들의 평균 키가 180 이상인 포지션 검색 */

SELECT "POSITION", MIN(WEIGHT), AVG(HEIGHT) FROM
	(
	SELECT * FROM PLAYER
	WHERE WEIGHT >= 80
	)
...
SELECT "POSITION", AVG(HEIGHT) AS POSITION_HEIGHT_AVG, (SELECT AVG(HEIGHT) FROM PLAYER) AS TOTAL_HEIGHT_AVG FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";
...
WHERE STADIUM_ID IN 
	...
	WHERE SCHE_DATE BETWEEN TO_DATE('2012-05-01', 'YYYY-MM-DD') AND TO_DATE('2012-05-02', 'YYYY-MM-DD')
	GROUP BY STADIUM_ID
	);
...
WHERE EMPLOYEE_ID IN (104, 107, 127);
...
/* EMP테이블의 SAL을 SALGRADE 테이블의 등급으로 나누기 */
...
SELECT /*E.HIRE_DATE, D.DEPARTMENT_ID , D.DEPARTMENT_NAME , D.MANAGER_ID , D.LOCATION_ID */ *
FROM EMPLOYEES E JOIN DEPARTMENTS D
ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
WHERE E.HIRE_DATE BETWEEN TO_DATE('2003-01-01', 'YYYY-MM-DD') AND TO_DATE('2005-12-31', 'YYYY-MM-DD')
...
SELECT /*p.TEAM_ID, m.MAX_HEIGHT, p.HEIGHT, p.PLAYER_ID ,p.PLAYER_NAME ,
p.E_PLAYER_NAME ,p.NICKNAME ,p.JOIN_YYYY ,p."POSITION" ,p.BACK_NO ,p.NATION ,p.BIRTH_DATE ,
p.SOLAR ,p.WEIGHT */ *
FROM PLAYER p JOIN
	(
...
WHERE p.TEAM_ID = m.TEAM_ID AND p.HEIGHT = m.MAX_HEIGHT
ORDER BY p.TEAM_ID;

/* EMPLOYEES 테이블에서 사원의 이름과 매니저 이름을 검색 */
SELECT d1.EMPLOYEE_ID, d1.FIRST_NAME || ' ' || d1.LAST_NAME AS EMPLOYEE_NAME, d2."MANAGER ID" , d2.MANAGER_NAME 
